...
| `/<orgId>/projects/sources` | `sources/sources-overview-section.tsx` | Connected Git accounts and repositories; the **connect-repository flow lives here**, not on the service editor (`sources/connect-repository-panel.tsx` owns the provider chooser, the installation/repository pickers, and the deploy-key lane). Connected accounts lists `git_provider_installation` rows with a **Suspended** badge; connected repositories list `source` rows with the repository-level **deploy on push** policy and a two-press Disconnect. Connect actions are `Linking.openURL(githubAppInstallUrl() | gitlabOauthConnectUrl())` — both endpoints answer **302** to the provider consent page, so they are navigated to, never fetched. Row expand fetches `GET /sources/:id` (`useSourceDetail`, `enabled: false` by default) purely for `reachabilityNote`; the webhook **URL** stays on `/admin/git`. Page override: `design-system/turbopanel/pages/sources.md` |
...
- **Compose** / **Services** (`ComposeBasePanel` + `sessionKey`): YAML editor or visual forms. **Discard** and **Save** appear only while dirty (hidden when clean); Save stays briefly as “Saving…” during persist. Discard is two-press (`Discard` → `Discard?`) in the surface header so it cannot grow the 40px chrome; it reseeds the draft from the last saved compose (invalid YAML is dropped). Overview Proposed/Saved uses the same pair. The Services form’s `SourceSection` only *binds* an existing `sourceId` — operators with zero connected repositories are sent to `projectSourcesHref` (`/<orgId>/projects/sources`), which owns the connect/install flow.
...
- `fetchSources()` → `GET /api/client/v1/sources` — `{ sources: SourceRecord[] }`, the org-owned Git repository bindings a compose service attaches to by `sourceId`. The list carries **no** webhook fields. Repository *connection* lives on `/<orgId>/projects/sources` (`projectSourcesHref`); the compose service editor’s `SourceSection` only *binds* an existing `sourceId` — it has no connect/install flow.
- `fetchSource(id)` → `GET /api/client/v1/sources/:id` — `{ source: SourceDetailRecord }`: the row plus instance-wide `webhookUrl` / `webhookReachable` / `reachabilityNote`, attached **only** for `github` / `gitlab` (a generic `git` source has no provider webhook), which is why the three are optional rather than nullable. Each call re-resolves the instance public-URL list, so `useSourceDetail` is `enabled: false` by default and a row opts in while expanded — never fetch it per listed row
- `createSource(body)` / `updateSource(id, patch)` → `POST` / `PATCH /api/client/v1/sources[/:id]` — `autoDeploy` (`immediate` / `checks_passed` / `disabled`) is a property of the **row**, so editing it affects every service bound to that repository
- `deleteSource(id)` → `DELETE /api/client/v1/sources/:id` — **409** `source_referenced_by_compose` (`SOURCE_REFERENCED_BY_COMPOSE_ERROR`) while a stored compose still names the source in `x-turbopanel.source.sourceId`; the UI renders that as "disconnect it from the service first", never as a retryable error
- `fetchGitInstallations()` → `GET /api/client/v1/sources/installations`; `fetchInstallationRepositories(installationId)` → `GET …/installations/:id/repositories` — every repository read mints a short-lived provider credential on the instance, so the picker only fetches once a connection is chosen
- `createGitlabDeployKey({ name })` → `POST /api/client/v1/sources/gitlab/deploy-keys` — the public half is returned **once** and never retrievable again; generate it before creating the source
- `githubAppInstallUrl()` / `gitlabOauthConnectUrl()` — browser-navigate targets (`GET …/sources/github/install` / `…/sources/gitlab/oauth`), **not** fetchable: both answer **302** to the provider's consent page and the operator has to land there to approve the grant. `fetch` would consume the redirect and show nothing
...
| `/admin/git` | `git-providers-section.tsx` | Instance-wide Git provider credentials — GitHub App (App ID / slug / client ID / private key / webhook secret) and GitLab OAuth application (application ID / secret / GitLab URL / redirect URI / webhook token). Secrets are write-only: the API reports presence only, so an untouched field is omitted from the PUT and the stored secret survives. Each panel shows its provider's webhook URL (from the configured public URLs) with a warning when the instance looks LAN-only |
...
- `GITHUB_WEBHOOK_PATH` / `GITLAB_WEBHOOK_PATH` mirror `turbopanel/src/surfaces.ts` by hand (different repo and runtime). `gitWebhookHint(origins, provider)` in `src/lib/git-webhook-url.ts` reproduces the instance's `webhookReachability()` rule: first publicly routable https origin wins, otherwise the first origin plus the LAN note. The unreachable notes point at a **manual deploy** (each service's declared branch), not at the `ref` field on `POST /environments/:id/deploy` — the control plane still answers `501 source_ref_unsupported` to any request that sets it. `WebhookEndpoint` in `git-providers-section.tsx` calls the helper **only on resolved data**: `usePublicUrlsOptional()` returning `undefined` renders a loading row and a real fetch failure renders an error notice, because feeding `[]` from an unresolved query would tell a configured operator they have no public URL (a 403 is different — the hook folds that into an empty list on purpose)
