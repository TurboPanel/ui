...
**Docs (for later click-through):** operator glossary at `https://turbopanel.io/docs/metrics`. Chart `id` values in this file (`cpu-pressure`, `memory-pressure`, …) are heading ids on `/docs/metrics/<group>#<chart-id>`. Event kinds deep-link to `/docs/metrics/events#<kind>`.
...
- **Overview tiles** (`StatTiles`) above the chart groups: CPU busy % (`host.cpu.busyPercent`, direct — no longer derived from idle), memory used % (`derived.memoryUsedPercent`), hosting storage used % (hosting-role filesystem, capacity-derived), total process count (`host.cpu.processCount` — `/proc` PID directories, not the run-queue `procs_running` gauge), uptime (from the separate `/metrics/connection` endpoint, not a plotted series) — latest values, not a chart. The v3 uplink tile is gone: v4's host contract has no host-level network-throughput field at all (only `tcpRetransmitPercent`/`softnetDropsPerSecond`)
- **Coverage strip** above chart groups: percent, gap count, resolution meta, amber gap hint — computed from the host series only
- **Collapsible groups:** CPU, Memory, Paging, Storage, Network, CPU detail, **Router**, **Storage usage**, **Docker**, Memory detail (host-scoped, all from the one host request) — followed by whichever presence-gated entity groups this server actually has (GPU, Network devices, Filesystems, Block devices, Physical signals, Ingress, Database proxy). All groups start expanded for now. **Docker** is the one host group hidden by *tier* rather than presence: on an entry-tier license (`tierPlacement.licenseTier` ranks 1 — `isEntryTierLicense`) the group is replaced by a one-line `InlineNotice` saying the tier does not include it, so the operator sees *why* rather than nothing. A missing tier (self-hosted / unassigned) keeps the group — the platform default plan grants the family.
...
Most entity ids (GPU/network/filesystem/block-device/hardware-signal identities) are not known ahead of time — they come back on the *response* as `inventory`, not something the client can enumerate up front. `managed.ingress`/`managed.database_proxy` are the exception: the daemon's `IngressAdapterId`/`DatabaseProxyAdapterId` are closed unions (`"caddy"`, `"proxysql"`) rather than topology-enumerated, so those two families' entity ids are always known and requested unconditionally — no discovery needed, no inventory entry required. A source that isn't actually running just comes back absent from the response's `entities[]`. The shared hosting **Traefik is no longer an ingress source** (v6): it is the host-wide singleton `managed.router` family with no entity id at all, read on the host-series path like `host.cpu`; `managed.storage` and `managed.docker` are host-wide singletons on that same path for the same reason (one hosting root, one Docker daemon per host).

So this screen makes a host query plus one or more entity queries per range:

1. **Host query** — the fixed list of host-singleton canonical ids (`HOST_METRIC_IDS`: `host.*`, `diagnostics.*`, `router.*`, `storage.*`, `dockerUsage.*` — collected automatically from every `hostMetric()` reference in the chart definitions), always enabled. Its response carries `host` (the plotted series) *and* `inventory`/`topologyGeneration` regardless of what was requested — `buildTopologyContextV4` runs unconditionally server-side.
2. **Entity queries** — built from the host query's `inventory` (every GPU/hardware signal, every **service** block device (`isServiceDevice`), every non-root filesystem, every network device with `role: 'other'`) plus the fixed `ingress`/`databaseProxy` source ids, enabled only once that combined id list is non-empty. The id list is split into batches of at most the server's 128-selector budget (`MAX_SERIES_METRIC_SELECTORS_V4`) — a single entity's fields always stay in one batch, but different entities of the same family may land in different batches — and every batch is fetched in parallel (`useServerMetricsSeriesBatches`) and merged before rendering, so a topology larger than one page of selectors never silently loses devices. Each batch's cache key folds in `topologyGeneration` so a mid-session topology change (a device added/removed) doesn't read a stale entity cache keyed only on the visible range.
...
- **CPU:** stacked-area chart of `host.cpu.{userPercent,systemPercent,iowaitPercent,stealPercent,softirqPercent}` (v4 dropped `nice`/`irq` — there is no per-mode residual bucket anymore); separate CPU-pressure (PSI, `pressureSomePercent`), busiest-core (`maxCoreBusyPercent`), and processes (`processCount` / `procsRunning` / `procsBlocked`) charts. No Load-average chart — v4's daemon contract carries no load-average metric at all, a deliberate capability loss (same class as v3's `load1/5/15`)
- **CPU detail:** context switches & interrupts, process forks, IRQ time, and **CPU frequency**. Frequency is a min–max area with the average drawn as the line on top.
- **Memory:** `host.memory.usedBytes` (raw) + `derived.memoryUsedPercent` (capacity-derived, server-side); `swapUsedBytes` + `derived.swapUsedPercent`; memory-pressure (PSI, some/full); swap I/O (in/out bytes/s); major page faults/s
...
- **GPU** *(presence: `inventory.gpus.length > 0`)*: per `gpuId` — utilization (memory-activity series only when the vendor reports it), memory used bytes, core/memory temperature, power draw, PCIe rx/tx throughput, throttle %. Titled from `vendor`/`chip`. Empty vendor fields use `hideWhenEmpty` on the card (or on the individual series when the chart still has another series with data)
...
- **Block devices** *(presence: `inventory.blockDevices.length > 0`)*: per device — throughput, IOPS, latency, utilization %, queue depth. Titled from `kernelName`/`model`. **No temperature chart here any more** — see Physical signals
- **Router** *(host-scoped `managed.router`, presence-gated per chart via `hideWhenEmpty`, not inventory-gated)*: backend requests / 5xx / retries, backend latency (avg), backends up vs total + services + routers, open connections, config reloads (delta counter), **time since the last config reload** (`configLastReloadAgeSeconds`, its own gauge chart in seconds via `formatDurationSeconds` — never on the reload-count axis; the legend's latest value is the "how stale is the running config" answer), soonest TLS expiry (a minimum across certs, not a mean). The group **header** carries a reachability readout whether or not the section is expanded: `GROUP_SUMMARY_SPECS.router` reads `router-backends`'s `up` / `total` series through `reachabilitySummary` as **`N of M`** backends reachable, with the mini bars plotting the reachable fraction and a chip that fires on a *low* figure (any backend down → Warning, none up while some exist → Critical; `0 of 0` is nothing configured, not an outage). This is the shared hosting Traefik that v5 folded into Ingress; v6 split it out because its service/router/backend view has no Caddy analogue
- **Storage usage** *(host-scoped `managed.storage`, presence-gated per chart)*: hosting / backups / Docker / logs as **used vs free** pairs (used = directory walk, free = the containing filesystem's), managed-database census (running vs healthy, connections used vs max — per engine, vendor series `hideWhenEmpty`). The Docker pair is special: the wire carries `dockerUsedBytes` (the total the Docker group's breakdown sums to) but **no `dockerFreeBytes`** — `StorageSample` is 19 fields with no spare slot — so `dockerStorageChart` joins the free side at render time from the **docker-role filesystem** (the host row's `rootFilesystemAvailableBytes` when Docker lives on root, else that filesystem's own `availableBytes` entity series, aligned by bucket index), which is exactly what the other three `*FreeBytes` fields mean. With no docker-role filesystem in the inventory the chart falls back to used-only. The group is also **joined with inode series** for the filesystems whose `inventory.filesystems[].roles` include any of `hosting` / `backup` / `docker` / `logs` (the wire has no `hosting_filesystem_id`; roles are the join key) — a role that lives on the root filesystem reuses Storage's `root-filesystem-inodes` chart, and one filesystem carrying several roles joins once. Distinct from **Storage** (host I/O + root capacity): that is "how is the disk behaving", this is "what is consuming it"
- **Docker** *(host-scoped `managed.docker`; **tier-gated** — hidden with a notice on the entry tier, see Layout)*: image layers, container writable layers, volumes, build cache — each with a `Reclaimable` series (what a prune would free: dangling images, unreferenced volumes, unused cache; never live objects) that `hideWhenEmpty`s when the vendor field is absent — plus object counts
- **Physical signals** *(presence: `inventory.hardwareSignals.length > 0`, replaces v3's fixed Hardware group)*: **drive and GPU temperatures live only here** — the `block`/`gpu` chart builders no longer define a temperature card; a drive's or GPU's thermal reading arrives as a `hardware.physical` signal whose `chip`/`component` metadata resolves to the owning device, and `attributedSignalTitle` (`src/lib/sensor-commands.ts`) captions the chart with that device's name (`nvme0n1 temperature`, `<vendor chip> core temperature`). On a VM the inventory has no signals, the group does not render, and no "no thermal data" panel is shown — absence is the answer. One chart per `signalId`, unit-aware (`celsius`/`watts`/`rpm`/`percent`/other) via the signal's own reported `unit`, with an optional threshold reference line from `.thresholds.critical` (preferred) or `.thresholds.warning`, titled with operator-facing names (`CPU package temperature` / `CPU package power`), not kernel `Package id 0` / `package-0`. Empty signals use `hideWhenEmpty` (do not show “Metric unavailable for this range”). This is where CPU/GPU/board/ambient/disk temperature, fan, and power readings all live now — v4 moved every physical sensor into this dynamic family instead of v3's fixed CPU-temperature/CPU-power/fan-speeds/disk-temperatures/ambient-board-temperatures slots. There is consequently no Tjmax/TDP headroom caption anymore: `cpuLimits`/`cpuThermalHeadroomPercent`/`cpuPowerHeadroomPercent` are still in the response envelope but v4 has no host-level CPU temperature/power field to pair them with (out of scope for the phase that shipped this cutover, flagged not fixed)
- **Ingress** *(presence: `entities['managed.ingress']` actually contains `caddy` this range — not inventory-gated)*: per source — requests & errors, responses by status (2xx/3xx/4xx/5xx, stacked), throughput (request/response bytes), **request latency p50 / p90 / p99** (three series read from `EntitySeriesPoint.derived.p50LatencyMs` / `p90LatencyMs` / `p99LatencyMs` — computed **server-side at read time** from the six cumulative `le` buckets; the old client-side mean-duration chart is gone, and there is deliberately no p999 series or toggle because six buckets cannot resolve it), requests-under-threshold buckets, upstreams & in-flight, retries. Titled from the fixed `caddy` → `Caddy` display map — `traefik` is not a source any more (it is the Router group)
...
- **System (`processCount`/`uptimeSeconds`)** group is gone as a standalone group — uptime now comes from `/metrics/connection`, not a metrics-store series; total process count is `host.cpu.processCount` (numeric `/proc` PIDs) and the run-queue pair stays `procsRunning`/`procsBlocked`
...
- Group expand/collapse: instant (no height animation); every group (host and presence-gated entity) starts expanded for now. Collapse still works; this is a temporary default while the layout is being reviewed.
...
- ❌ Rendering an empty series on a mixed chart when that series has `hideWhenEmpty` (Memory activity on Intel iGPU)
...
- ❌ Treating `ingress`/`databaseProxy` `sourceId` as open-ended or inventory-gated — it's a fixed closed set (`caddy`/`proxysql`), always requested; gate group visibility on whether the source actually appears in the response, not on a topology inventory entry
- ❌ Re-adding a `traefik` ingress source, or reading router figures off the entity path — the router is host-scoped `managed.router`
- ❌ Requesting `storage.dockerFreeBytes` (or any field not on `StorageSample`) — the Docker free side is a client-side join off the docker-role filesystem, never a wire selector
- ❌ Plotting the router's config-reload *age* (a gauge in seconds) on the reload *count* chart (a delta counter), or treating a high `N of M` as the fault — reachability chips fire on a low figure
- ❌ Deriving latency percentiles (or a mean) client-side from the buckets / duration sum — read `derived.*`; and no p999
- ❌ Hiding the Docker group because the response lacked the family — gate it on the license tier so the reason is visible; hide-by-absence is only for per-chart `hideWhenEmpty`
- ❌ A drive/GPU temperature card outside Physical signals, or a "no thermal data" placeholder on a VM
