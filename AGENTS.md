...
- `fetchDeployPreview(environmentId)` → `GET /api/client/v1/environments/:id/deploy-preview` — `DeployPreviewResponse` (`composeFiles[]` ordered `-f` chain with `filename` / `role` / `content` and optional provenance `source` / `path`, deprecated merged `composeYaml` fallback, `projectName`, `containers[]`, `volumes[]`, `warnings[]`); secret values redacted; same prepare path as deploy
