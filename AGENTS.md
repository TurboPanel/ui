...
Managed environments use the shell **Environments** tab (`project-environments-section.tsx`). Compose projects have no Environments section tab — env work lives under the selected environment chip; Networking / Storage are reached via the environment-scope Settings rows in `ProjectSettingsArea` (`EnvironmentDetailBody` embedded there), not standalone section chips. `/networking` and `/storage` redirect to the current scope path only when an environment scope is already active; otherwise they fall back to Overview (they must not invent the first environment on a cold load). Hosting deep links `/networking/:id` preserve `?hostingId=` on that redirect so Settings can expand and focus the matching hosting row. The shell environment selector keeps the active environment across managed non-Overview tabs; compose Overview owns the unified chip strip (path `/environments/:id` selects that env).

**Overview Compose panel** (`compose-tabs.tsx` → `ComposeServicesTab` + `overview-environments-panel.tsx`):

- **View ⇄ edit mode:** Overview defaults to a read-only saved-compose card (`ComposeSavedView`) — scope title, summary chips (services / networks / volumes / server), optional running-service status list, and `ReadOnlyYamlBlock`. **Edit** (when `canManage && projectAllowsMutations`) mounts the existing Compose/Services editor (`ComposeBasePanel`); successful save returns to view. **Discard Changes** leaves edit without saving. Scope switches reset edit mode.
- Editor chrome (edit mode): one bordered compose surface. Header row: quiet **Compose / Services** underline tabs (left; internal view id `visual` = form cards — leave "Visual" free for a future topology tab); right-aligned **Discard Changes** + **Save**. **Set Default Project Server (Optional)** stays on Settings. Scope chips (**Project · environments**) live in the project header (`ProjectScopeSelector`), not the compose toolbar. Delete controls live only in the Settings area — compose projects have **no** header trash (managed projects keep the header trash). Lifecycle Deploy / Redeploy / Start / Stop / Refresh / Destroy and env add live below via `OverviewEnvironmentsPanel` when an environment is selected. Env-only server pin appears when the selected env has no effective server. Status is loaded with one `fetchContainers({ environmentId })` per environment on mount / when the environment-id list changes, and again on explicit **Refresh** or after a tracked command reaches a terminal status — **no auto-poll / `refetchInterval`**.
- **Scope banner** (`ComposeScopeBanner` in `src/components/org/project/compose-scope-banner.tsx`): **Project** scope has no banner (saved view title is **Compose - Project**). **Environment** scope shows inheriting (with **Create override** / **Start from project compose**) vs overriding (with **Clear overrides**, two-press) plus an inherited-services chip hint. **Edit** on an inheriting environment enters edit mode on the blank overlay (same as Create override); Start from project / Clear overrides stay on the banner.
- **Project vs environment selection:** Overview Project compose is `/overview` (no query). Selecting an environment chip navigates to `/environments/:environmentId` — that environment is highlighted (not Project) and Overview shows its overlay / lifecycle. Post-setup navigation opens `/overview`. While Project is selected, Networking / Storage settings rows are environment-scoped only; context may still keep a concrete `selectedEnvironmentId`. Project mode hides lifecycle actions.
- **Preview Deployment** (`PreviewDeploymentModal` in `src/components/org/project/preview-deployment-modal.tsx`): opens only when deploying / redeploying an environment — Overview **Deploy** / **Redeploy** / **Cacheless redeploy**, and Environments-tab **Deploy**. Not shown for project-level compose editing or lifecycle Start/Stop of already-deployed containers. Modal title **Preview Deployment**; Merged / Prepared toggle (Prepared defaults on open). **Merged** — client-side `mergeComposeOverlay(project, environment)` with `x-turbopanel` hidden from the YAML block (native Docker Compose only; target server is in the modal subtitle); **Prepared** — `useDeployPreview` fetch-on-open (no auto-poll). Confirm enqueues deploy (`confirmLabel` matches Deploy / Redeploy / Cacheless redeploy).
- **Settings area** (`ProjectSettingsArea` in `src/components/org/project-settings-area.tsx`): quiet **Add …** chips plus a **Settings** chip below the compose panels (not a stack of empty sections); scope-aware header text. **Project scope:** **Add Server / Add Variable / Add System user** reveal resource sections (also auto-show when data exists); **Settings** reveals Workspace, container naming, and Danger → Delete project (`ProjectDeletePanel`). Container naming is a single **Keep original container names** toggle (default Off = rename to service UUIDs; On = `options.containerNaming: 'custom'`) with a warning that keeping names disables rolling updates. **Environment scope:** **Add Server / Add Network / Add Storage** with the same reveal pattern; **Settings** reveals Danger → Delete environment (two-press `useDeleteEnvironment`; disabled with a pointer to Project → Settings when only one environment exists).
- **Deploy / Redeploy / Start / Stop / Refresh / Destroy** — refined quiet toolbar buttons (not large primary chips). **Deploy** (no host-deployed containers) and **Redeploy ▾** (deployed) open **Preview Deployment** then `deployEnvironment` (optional `{ noCache: true }` for **Cacheless redeploy**). **Start** appears only when deployed-but-stopped and calls `runEnvironmentLifecycle(id, 'start')` with no preview modal. Stop keeps files/data. Destroy is two-press confirm (`stopEnvironment`) and removes containers **and their data volumes**. Deploy ack conflicts surface plain-language copy pointing at the Environments tab.
- **Delete** — compose **Settings → Danger** (`organization:own`): with more than one environment, two-press confirm deletes the selected environment; project delete uses `ProjectDeletePanel`. Managed projects keep the header trash control. Name validation via `validateEnvironmentName` for Add.
...
- The active environment renders `EnvironmentDetailBody` (exported from `environment-detail-section.tsx`) with `embedded` set, which hides the redundant "Environment" meta panel but keeps **required server dropdown** (`EnvironmentRecord.serverId`), merged runtime compose, **Deploy** (opens **Preview Deployment** modal — not an always-visible preview panel), hostnames + TLS + **bind** (`public` / `datacenter` / `local`) + optional public **`ipId`** picker (when bind is public), **per-hosting variables** (embedded `VariablesSection` with `{ hostingId }` after the hosting row is saved), **container status**, and environment-scoped variables. **Compose overlay** is hidden when embedded for Settings Networking (`showComposeOverlay={false}`) — edit environment overlays on Overview (`/environments/:id`) via the scope banner / compose editor (**Create override** / **Clear overrides**). Switching tabs remounts the body via `key={environmentId}`. Compose projects expose **Keep original container names** (`options.containerNaming`: `uuid` default vs `custom`) under Project → Settings.
...
- `createTlsCertificate(body)` → `POST /api/client/v1/tls` (`upload` | `self_signed` | `lets_encrypt`); upload sends PEM once; server seals the key as `enc`
...
- Types: `ProjectRecord` (`metadata.type`, `options.compose`), `EnvironmentRecord` (`metadata`, `options.compose`), `CatalogSummary`, `VariableRecord`, `CreateProjectBody`, `ServiceRecord` (`displayName`, `composeServiceName` — compose key is derived-only; never send on create/update), `ContainerRecord` (`serviceId`, `serverId`, top-level `containerId` / `containerName` / `status` / `composeServiceName` / `role` (`'service' | 'ingress' | 'system'`, allocator-owned — `service` is an ordinary workload container, `ingress` is the per-service Traefik at ordinal 1 named `<serviceId>-in`, `system` is a `turbopanel-system` platform component) — status/id from Postgres reconcile, never DO reads; residual `metadata` only), `DeployPreviewResponse.containers[]` also carries `role`, `TlsRecord` (`source`, `metadata`, `certificatePem` — no private key), `DatacenterRecord`, `IpRecord` (`vpnId`, `scope` includes `vpn`, derived `version`), `NetworkRecord` (`kind` without `vpn`, `cidr`, …), `VpnRecord` (`cidr`), `PeerRecord` (`role`, `endpointIpId`, `tunnelIpId`, no `presharedKey`), `HostingRecord` (`tlsId?`, `ipId?`), `OrgServerRecord` / `ServerDetailRecord` (`datacenterId`, `datacenterDisplayName`)
...
- **Principal password write-only rule:** principals are not a public org-console surface. Hosting/database-user flows create them behind the scenes; passwords are sealed as `enc` at rest and must never be displayed or pre-filled after the optional show-once generate step.
...
| `/admin/secrets` | `secrets-reencrypt-section.tsx` | Superadmin **Re-encrypt secrets** sweep — re-seals at-rest `enc` blobs (variables, TLS keys, principal passwords) onto the current data-encryption key version |
