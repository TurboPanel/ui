...
| `/<orgId>/projects/new` | `project-create-section.tsx` | Create **wizard** (`project-create/`): Details (name, description, existing/new workspace) → **Next** → Type → compose draft *or* catalog pick → **Create project**. Nothing is written until Create — no half-made project to delete after a mis-click. `?type=` skips the type cards; `?workspaceId=` preselects the workspace |
