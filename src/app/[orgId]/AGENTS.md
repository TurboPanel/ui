...
| `/<orgId>/billing` | `billing-section.tsx` | **Hosted only** — gated on `GET /status` `billingEnabled` (`useAuth().billingEnabled`): the sidebar omits the entry and the route `Redirect`s to Overview when false; never probe `/billing/*`. First checkout (tier table + `nproc && free -g` sizing hint → hosted Checkout), seats vs licenses per tier (`StatTiles`), add/release seats, move one **license** between tiers (`licenseId` + `targetTierId` — upgrades preview then apply with the quoted `prorationDate`; downgrades defer to the period end), Customer Portal for invoices / payment method. `?tier=&license=` deep link from the server detail Upgrade button; Stripe returns with `?checkout=success\|cancel`. Design: `design-system/turbopanel/pages/billing.md` |
