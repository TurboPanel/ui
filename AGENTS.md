...
| Legacy `/servers/networks` · `/servers/datacenters` · `/servers/ips` · `/servers/vpns` (+ detail paths) | thin `<Redirect />` screens | Map to `/network`, `/network/sites/:id`, `/network/addresses`, `/network/links/:id` — preserve params and `?serverId=` |
