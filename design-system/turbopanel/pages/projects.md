...
- **Inheriting environment**: `InlineNotice` (**Using project compose** + Create override / Start from project compose — the copy says "using", never "inheriting") above the project compose rendered plainly and readably in a `Project compose · read only` block. Never a card over a dimmed backdrop — the point of the screen is reading what deploys today.
...
- **Overview** (`/overview` or `/environments/:id`): opens on **Project** by default. Inventory strip + topology **diagram only** (no Diagram/YAML toggle; no Edit button). YAML is the Compose tab.
  - **Inventory strip** (`ComposeInventoryStrip`) — one compact inline row of `icon count LABEL` groups across the top of the Overview body (not tiles; the counts spend a single line of height so the diagram stays the content of the surface), counting resources at the active scope: Project scope shows environments / servers (distinct, across environments) / services / networks / volumes / storage; Environment scope shows server (0/1) / services / networks / volumes / storage / bindings, all scoped to that environment (no N+1 fetches). Glyph per resource from `icons/resource-icons.tsx` (+ the compose nav glyphs); a zero count dims rather than disappearing, so the set of resource types stays legible. An environment with no compose overrides ends this row with a quiet **Using project compose** caption (plain muted text, not an `InlineNotice`). Not the wider bordered `StatusStatBoxes` — those are page-level fleet numbers.
  - **Diagram** (`ComposeGraphView`, `src/lib/compose/graph.ts`) — Overview body: cross-platform `react-native-svg` node graph (services as cards with a status dot when deployed and a faint uppercase SERVICE / SITE kind tag, networks as pill "bus" connectors, volumes as dashed resource tags), laid out top-to-bottom by `depends_on`, with accessibility text fallback (`describeComposeGraph`) and a **key** rendered as one quiet hairline pill below the canvas, right-aligned (diagram chrome, not a second content block — node shapes already carry their own kind label; Hosting / Server key entries appear only when drawn). Live deployment facts extend the topology into the environment picture: per-service hostnames (`ComposeDocFacts.byService[].hostname`) draw **hosting** nodes above the service layers with an arrowed edge into the service they serve, and the effective server placement (`placementLabel` — env pin, else project default, i.e. where the scope deploys or *will* deploy) wraps services + networks/volumes in a mermaid-subgraph-style **server frame** captioned `SERVER · name`; hosting sits outside the frame because traffic enters from beyond the server. The diagram centres in its surface rather than pinning left. Empty when there are no nodes; blank compose shows “No compose defined yet.”
...
- Lifecycle **Preview ▾ / Deploy / Redeploy ▾ / Start / Stop / Refresh / Destroy** sits below when an environment is selected, with a **Server** pin immediately beside Deploy. After start, service status rows appear on Overview with green / yellow / red dots (mirrored on diagram service nodes).
