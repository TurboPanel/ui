...
| `/admin/git` | `git-apps-section.tsx` | **Instance-wide** Git provider applications — a *collection*, not a pair of singletons: several GitHub Apps and GitLab OAuth applications may coexist (a github.com App and a GitHub Enterprise one, separate Apps per customer account, gitlab.com beside a self-managed GitLab). Each row shows its own webhook URL, which carries that app's routing ref so a delivery names its app before any secret is checked. Secrets are write-only: the API reports presence only, so an untouched field is omitted from the PATCH and the stored secret survives. **Create a GitHub App** runs the manifest flow — GitHub creates the App with the scoped webhook URL already set and hands back every credential in one hop. The same component serves `/[orgId]/projects/git-apps` with `scope="org"`, where an organization manages its own apps and sees instance-wide ones as `readOnly` |
