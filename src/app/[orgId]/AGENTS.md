...
Main product shell for signed-in users. Web keeps the sidebar + narrow-viewport drawer (`org-shell.tsx`); iOS/Android use `org-shell.native.tsx` with `OrgTabBar` (Overview, Projects, Servers). `ORG_TAB_AREA_IDS` is the native tab set; Managed / Network / Access / Billing / Workspaces / Admin / Pending keys remain deep-link-only on native for now. **Manage Organization** is not a sidebar area — reach it from the header org switcher **Manage** button, a gear on `/organizations`, or a deep link.
...
- `src/components/org/org-sidebar.tsx` — area nav + sub-routes for the active area (web); filters out the `billing` area unless `useAuth().billingEnabled`
...
| `/<orgId>/servers/datacenters/[datacenterId]` | `datacenter-detail-section.tsx` | Name/description; Subnets (rows + Add subnet); Routing (address preference + **priority** 0–1000 lower-wins with a “which datacenter currently wins” hint + **Trusted/Untrusted** `Toggle`, one merged `options` write); member pins (server + IP dropdowns, both families, only IPs inside this datacenter’s subnets; **Stale** badge when the daemon stopped reporting the pinned address); TurboFabric relays; timezone; two-press delete when empty |
...
| `/<orgId>/network` | `network/network-overview-section.tsx` | Hub linking Datacenters, TurboFabric, Addresses, Docker networks, and Reserved ranges (private subnet CRUD lives on Datacenters) |
| `/<orgId>/network/addresses` | `network/network-addresses-section.tsx` | Org address pool (`public` / `datacenter`) |
| `/<orgId>/network/docker` | `network/network-docker-section.tsx` | Docker external network registry for compose (optional subnet / IP range / gateway / MTU per registration) + org-wide **Host address pools** (`GET`/`PUT /organizations/:id/docker-networking` — dockerd `default-address-pools` + `bip`; saving restarts dockerd on every host) |
| `/<orgId>/network/reserved` | `network/network-reserved-section.tsx` | Reserved ranges (`kind: 'reserved'`, org-scoped, CIDR-required): CIDRs something outside TurboPanel routes, which no allocator may hand out. Add / rename / two-press delete; collision 409s through `network-error-copy.ts` |
...
| `/<orgId>/projects/git-sources` | `git-sources/git-sources-section.tsx` | The organization's Git **applications** — a collection, not a pair of singletons. Instance-wide apps appear here as `readOnly` rows beside the org's own. **Create a GitHub App** opens `git-sources/github-app-wizard.tsx` in place (never a modal); **Add manually** opens `git-sources/forge-editor.tsx` for an App or OAuth application registered by hand. This surface deliberately does **not** list repositories — a `repository` row is created when a project attaches one; the rows are listed and managed at `projects/repositories`. The same component serves `/admin/git` with `scope="admin"`. Page override: `design-system/turbopanel/pages/git-sources.md` |
| `/<orgId>/projects/repositories` | `repositories-section.tsx` | The organization's connected Git repositories — one deduped row per repository (canonical clone URL, server-enforced). Shows the auth lane (connection account / deploy key / anonymous), the tracked branch with provider drift (`metadata.detectedDefaultBranch`), auto-deploy, and which projects use the row (`project.repositoryId`). **Refresh** re-reads provider facts (`POST /repositories/:id/refresh`, connection rows only); **Delete** (two-press) is offered only for unused rows, with the server 409 `source_referenced_by_compose` as the authority. No add form here — repositories are attached from project flows. Page override: `design-system/turbopanel/pages/repositories.md` |
...
| `/<orgId>/billing` | `billing-section.tsx` | **Hosted only** — gated on `GET /status` `billingEnabled` (`useAuth().billingEnabled`): the sidebar omits the entry and the route `Redirect`s to Overview when false; never probe `/billing/*`. Vocabulary is **servers** and **licenses** (never "seat"); a license is plumbing, never an object, and the UI never assigns a tier. First checkout (tier table + `nproc && free -g` sizing hint → hosted Checkout), a **Licenses** summary (`licenses.held` of `purchased` in use, `available` more servers can be added, `releasing` leaving at period end), **servers not covered** notice from `servers[]` where `assignedTierId` is null (names the server and its `requiredTier`), a per-tier table from `tiers[]` with **+1** (preview → `seats` delta +1, invoiced now) and **−1** (delta −1, released at period end; **409** `servers_uncovered` names which server needs which tier) plus a buy-at-another-tier picker, **Move a license** from tier → to tier (`{ fromTierId, toTierId }`: up = preview then apply with the quoted `prorationDate`; down = scheduled for the period end), pending changes with `landsAt`, Customer Portal for invoices / payment method. `?tier=` deep link (id or label) from the server detail button preselects the tier to buy at or move to; Stripe returns with `?checkout=success\|cancel`. Design: `design-system/turbopanel/pages/billing.md` |
