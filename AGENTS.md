...
- **Sign-in** — `/sign-in` after install; superadmin email + password only (host accounts cannot sign in). Layout: centered column via `AuthScreenShell` (`src/components/auth/auth-screen-shell.tsx` + `auth-form-styles.ts`), form `maxWidth` 400 — **Sign In** title above the form panel (accent top edge on the panel), `© {year} TurboPanel` copyright below. Shell backdrop (`AuthScreenBackground`): performant static wash/grid/vignette (`auth-grid-layer` — CSS on web, tiled SVG on native) + 2×2 Reanimated accent streaks on random grid lines via shared values (skipped when reduced motion). Fields use floating labels (`AuthFloatingField`): resting label inside the field, shrinks to the top on focus/value; password toggle is eye / eye-slash icons (`auth-eye-icons.tsx`). **Auth accent by runtime** (`src/lib/auth-accent.ts` + `GET /api/client/v1/status` `runtime`): Workers / HA → blue `#3366cc`, Deno self-hosted → green `#3dd68c`. Dual brand tokens in `src/lib/theme.ts` (`green` / `blue` / `accent`); page override in `design-system/turbopanel/pages/sign-in.md`.
