...
- `fetchSources()` → `GET /api/client/v1/sources` — `{ sources: SourceRecord[] }`, the org-owned Git repository bindings a compose service attaches to by `sourceId`. The list carries **no** webhook fields. A row is created by `attachSource()`, not by an operator managing repositories on a page of their own — there is no such page.
