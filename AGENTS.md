...
| `/<orgId>/projects/new` | `project-create-section.tsx` | Create **wizard** (`project-create/`): Details (name, description, existing/new workspace) → **Next** → Type → compose draft *or* catalog pick → **Create project**. Nothing is written until Create — no half-made project to delete after a mis-click. Type cards are **Compose · Services · Template · Managed**; Compose and Services are both `docker-compose` and differ only in which compose-surface tab opens (`SetupTypeOption.section`). Key the cards off `option.choice`, never `option.type` — two cards share a type, so `type` yields duplicate React keys. Card copy must not imply TurboPanel hosts or runs anything (it is self-hosted; Managed means we configure the engine on the operator's own servers). The **compose step is the project screen itself**, not a wizard copy of it — see **Draft project surface** below. `?type=` skips the type cards; `?workspaceId=` preselects the workspace |
...
Managed environments use the shell **Environments** tab (`project-environments-section.tsx`). Compose projects have no Environments section tab — env work lives under the selected environment chip. Hosting and Servers are compose surface tabs (`ComposeHostingTab` / `ComposeServersTab` in `compose-resource-tabs.tsx`) on both Project and environment scope. Storage is reached via the environment-scope settings gear on `ProjectScopeSelector` (`EnvironmentSettingsPanel`). The shell environment selector keeps the active environment across managed non-Overview tabs; compose Overview owns the unified chip strip (path `/environments/:id` selects that env).

**Overview Compose panel** (`compose-tabs.tsx` → `ComposeServicesTab` + `overview-environments-panel.tsx`):

- **Surface tabs inside the editor chrome:** one bordered compose surface hosts underline **Overview · Compose · Services · Hosting · Servers** (`ComposeSurfaceSectionTabs` — not a shell strip above the surface). Paths: `/overview`, `/compose`, `/services`, `/hosting`, `/servers` (Project); `/environments/:id` plus `…/compose` / `…/services` / `…/hosting` / `…/servers` (environment). Create-wizard drafts omit Hosting / Servers (`DRAFT_COMPOSE_PROJECT_TAB_IDS`). Scope chips (**Project · environments**) stay in the project header and **keep the active surface tab** when switching. No Edit button; no Diagram/YAML toggle.
...
- **Servers tab** (`ComposeServersTab`): Project scope shows **Default project server** (`ProjectServerHeaderControl` / `options.defaultServerId`) plus a pin panel per environment; environment scope is that env’s pin (`updateEnvironment({ serverId })`, clear inherits the project default). Delete controls live only in the scope-chip settings gear — compose projects have **no** header trash (managed projects keep the header trash). Lifecycle **Preview ▾ / Deploy / Redeploy ▾ / Start / Stop / Refresh / Destroy** and env add live below via `OverviewEnvironmentsPanel` when an environment is selected. Env-only server pin still appears on Overview when the selected env has no effective server. Status is loaded with one `fetchContainers({ environmentId })` per environment on mount / when the environment-id list changes, and again on explicit **Refresh** or after a tracked command reaches a terminal status — **no auto-poll / `refetchInterval`**.
...
- **Scope-chip settings** (`ProjectSettingsPanel` / `EnvironmentSettingsPanel` in `src/components/org/project-settings-area.tsx`, opened from the gear on each `ProjectScopeSelector` chip): full controls live in the dropdown panel (not below compose). **Project gear:** quiet **Add Variable / Add System user** chips plus Workspace, container naming, and Danger → Delete project (`ProjectDeletePanel`). Container naming is a single **Keep original container names** toggle (default Off = rename to service UUIDs; On = `options.containerNaming: 'custom'`) with a warning that keeping names disables rolling updates. **Environment gear:** **Add Storage** plus Danger → Delete environment (two-press `useDeleteEnvironment`; disabled with a pointer to Project settings (gear) when only one environment exists). `?hostingId=` on an environment path opens the **Hosting** tab (keeps `?hostingId=` so `EnvironmentDetailBody` can focus the row).
