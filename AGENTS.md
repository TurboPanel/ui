...
- **Settings area** (`ProjectSettingsArea` in `src/components/org/project-settings-area.tsx`): quiet **Add …** chips below the compose panels (not a stack of empty sections); scope-aware header text. **Project scope:** **Add Server / Add Variable / Add System user** reveal resource sections (also auto-show when data exists); **Workspace** is its own always-visible area (project membership); Container naming + Danger → Delete project (`ProjectDeletePanel`). **Environment scope:** **Add Server / Add Network / Add Storage** with the same reveal pattern; Danger → Delete environment (two-press `useDeleteEnvironment`; disabled with a pointer to Project settings when only one environment exists).
