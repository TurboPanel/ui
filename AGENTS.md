...
| `/<orgId>/servers/datacenters` | `datacenters-overview-section.tsx` | Org datacenter inventory (member pin counts, private CIDRs). A datacenter **is** one private CIDR. Empty list + **+ Datacenter** (no auto-opened create form). **+ Datacenter** when ≥1 server reports a private IP; opens `/servers/datacenters/new` |
...
- `fetchDatacenters()` / `fetchDatacenter(id)` / `createDatacenter` / `updateDatacenter` / `deleteDatacenter` / `addDatacenterMembers` / `removeDatacenterMember` → `/api/client/v1/datacenters`; create body is `{ displayName?, description?, members: [{ serverId, address }], sourceServerId? }` (CIDR is derived from the seed member’s reported private `ips[].cidr` when present, otherwise a typical LAN `/24` or `/64` — never typed); list **and** detail include `DatacenterRecord.privateCidrs: string[]` (always present, default `[]`); `fetchDatacenterNameSuggestions()` derives editable names from server geo/ASN metadata; delete returns **409** `datacenter_has_members` until every server is unassigned (`DATACENTER_HAS_MEMBERS_ERROR`), then **409** `datacenter_has_networks` only if leftover non-site networks remain.
- `fetchIps(filters?)` / `fetchIp` / `createIp` (never send `version`) / `updateIp` / `deleteIp` → `/api/client/v1/ips`; `IpScope` is `'public' | 'datacenter'` (no `loopback`, no `vpn`); `version` is server-derived/read-only; delete surfaces **409** `ip_in_use` as "This address is pinned to a hosting — unassign it first."
...
- Error constants: `IP_IN_USE_ERROR`, `GATEWAY_DATACENTER_REQUIRED_ERROR`, `GATEWAY_DATACENTER_CIDR_REQUIRED_ERROR` (relay PATCH), `FABRIC_RECONCILE_FAILED_ERROR` / `FABRIC_RECONCILE_PENDING_ERROR` (deploy), `DATACENTER_HAS_MEMBERS_ERROR` / `DATACENTER_HAS_NETWORKS_ERROR` (datacenter delete), `DATACENTER_IP_REQUIRED_ERROR` / `PRIVATE_PATH_UNAVAILABLE_ERROR` (managed replica overlay — live instance codes from `private-endpoint.ts`; `PEER_TUNNEL_ADDRESS_REQUIRED_ERROR` in `instance-api.ts` / `managed-services.ts` is a dead constant, code follow-up)
