...
- **What will run** (`EffectiveComposePanel`): segmented **Merged** / **Prepared** toggle. **Merged** — client-side `mergeComposeOverlay(project, environment)` + display-only `withEffectivePlacement`, rendered via `ReadOnlyYamlBlock`; available even without a server pin or on Project scope (mirrors `../instance/src/lib/compose/AGENTS.md` merge semantics). **Prepared** — unchanged `DeployPreviewPanel` / `useDeployPreview` fetch-on-demand path (no auto-poll); requires an environment + effective server pin; disabled on Project scope.
- **Settings area** (`ProjectSettingsArea` in `src/components/org/project-settings-area.tsx`): collapsed-by-default sections below the compose / effective-compose panels; scope-aware header text. **Project scope** (`ProjectSettingsSections`): Servers, Variables, Container naming, Workspace, System users, Danger → Delete project (`ProjectDeletePanel`). **Environment scope** (`EnvironmentSettingsSections`): Server, Networking, Storage, Danger → Delete environment (two-press `useDeleteEnvironment`; disabled with a pointer to Project settings when only one environment exists).
