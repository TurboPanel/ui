...
- `fetchIps(filters?)` / `fetchIp` / `createIp` (never send `version`) / `updateIp` / `deleteIp` → `/api/client/v1/ips`; `IpScope` is `'public' | 'datacenter'` (no `loopback`, no `vpn`); `version` is server-derived/read-only; optional `description` (`varchar(255)`, not a display name); delete surfaces **409** `ip_in_use` as "This address is pinned to a hosting — unassign it first."
